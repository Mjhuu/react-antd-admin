 ...
 * @param {Function}fun
 */
export function uploadAliOss(fileArr, user, client, fun) {
  let successCount = 0; //上传完成个数
  for (let i = 0; i < fileArr.length; i++) {
    //0 -->未上传 1-->上传中  2--->已完成
    if (fileArr[i].uploadState === 2) {
      console.log(`${fileArr[i].file.name}已上传`);
    } else {
      let file = fileArr[i].file;
      let time = new Date();
      let fileName = time.getTime() + '_' + user + '.' + file.name.substring(file.name.lastIndexOf('.') + 1);
      client.multipartUpload(fileName, file, {
        progress: p => {
          fileArr[i].uploadState = 1;
          fileArr[i].percent = parseInt(p * 100);
          if (fileArr[i].percent === 100) {
            fileArr[i].uploadState = 2;
          }
        },
      }).then(result => {
        let updateFileName = 'https://' + result.bucket + '.' + 'oss-cn-qingdao.aliyuncs.com' + '/' + result.name;
        console.log(i + ':' + updateFileName);
        fileArr[i]['uploadUrl'] = updateFileName;
        successCount += 1;
...
export function downloadFile(fileName, filePath) {
  let name = `${fileName}${filePath.substring(filePath.lastIndexOf('.'))}`;
