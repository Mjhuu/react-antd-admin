...
import {getArticle, collectArticle, commentArticle, gradeArticle, getArticleInfo, delComment} from "../../Api";
import Typing from "../../Components/Typing/Typing";
import {
    Avatar, Button, Comment, Drawer, Empty, Icon, Input, message, Modal, Pagination, Rate, Skeleton, Tag, Tooltip
} from "antd";
...
class Software extends Component {
    ...
    render() {
        let {total, articleList, limit, page, reqArticleLoading, commentList, expandIds, placeholder, replyContent, replyPId, selectArticleDrawer, selectArticleInfo, score, desc, commentContent} = this.state;
        let skeleton = [];
        ...
        return (
            <div className="software-box">
                        ...
                        <section className="select-article-info">
                            <div className="top">
                                <div className="headImg">
                                    <Avatar style={{cursor: 'pointer'}} src={selectArticleInfo.adminInfo.headImg ? selectArticleInfo.adminInfo.headImg : HeadImg} size={40} icon="user"/>
                                    <div className="aside">
                                        <div className="name">
                            ...
                            <div className="bottom">
                                ...
                                </div>
                                {
                                    ...
                                    <div className="comment-list">
                                        ...
                                        {
                                            JSON.stringify(commentList) === '{}' ? <Empty description="暂无评论" /> : Object.keys(commentList).sort((a, b) => b - a).map((v, index, arr) =>{
                                                return <Comment
                                                    key={v}
                                                    author={<span style={{ fontSize: 16 }}>{commentList[v].adminInfo.username} {selectArticleInfo.adminInfo.uuid === commentList[v].adminInfo.uuid && <Tag color="#87d068">博主</Tag>}</span>}
                                                    avatar={<img className='avatar-img' src={commentList[v].adminInfo.headImg ? commentList[v].adminInfo.headImg : HeadImg} alt='avatar' />}
                                                    content={<div className='info-box braft-output-content' dangerouslySetInnerHTML={{__html: commentList[v].commentInfo.content}} />}
                                                ...
                                                >
                                                    {
                                                        commentList[v].children.slice(0, expandIds.includes(commentList[v].commentInfo.id) ? commentList[v].children.length : 1).map((vv, childrenIndex)=>
                                                            <Comment
                                                                ...
                                                                author={<span style={{ fontSize: 15 }}>{vv.adminInfo.username} {vv.adminInfo.uuid === selectArticleInfo.adminInfo.uuid && <Tag color="#87d068">博主</Tag>} {
                                                                    vv.replyAdminInfo && <span style={{color: '#eb7350', cursor: 'pointer'}}>@{vv.replyAdminInfo.username} {vv.replyAdminInfo.uuid === selectArticleInfo.adminInfo.uuid && <Tag color="#87d068">博主</Tag>}
                                                                    </span>                               }</span>}
                                                                avatar={<img className='avatar-img-small' src={vv.adminInfo.headImg ? vv.adminInfo.headImg : HeadImg} alt='avatar' />}
                                                                content={<div className='info-box' dangerouslySetInnerHTML={{__html: vv.commentInfo.content}} />}
                                                                actions={this.renderActions(vv, commentList[v].commentInfo.id, 2, childrenIndex)}
                                                            />)}
                                                    <div className='toggle-reply-box' style={{ display: commentList[v].children.length > 1 ? 'block' : 'none' }}>
                                                        {
                                                            expandIds.includes(commentList[v].commentInfo.id) ? (
                                                                <span onClick={() => this.foldReply(commentList[v])}>收起全部{commentList[v].children.length}条回复 <Icon type='up-circle' /></span>
                                                            ) : (
                                                                <span onClick={() => this.expandReply(commentList[v])}>展开全部{commentList[v].children.length}条回复 <Icon type="down-circle" /></span>
                                                            )
                                                        }
                                                    </div>
                                                    {replyPId === commentList[v].commentInfo.id && (
                                                        <div style={{ width: '70%', textAlign: 'right' }}>
                                                            <TextArea onChange={e => this.commentChange(e, 'replyContent')} rows={4} style={{ marginBottom: 10 }} value={replyContent} placeholder={placeholder} />
                ...
                <Typing>
                    <p className="ant-alert ant-alert-info ant-alert-no-icon">这里汇集了众多好用的软件，如果你有实用的软件可以分享【点击左侧"写文章"】上来。</p>
                </Typing>
                            ...
                            reqArticleLoading ? skeleton.map(v=> <Skeleton key={v} active />) : <ul>
                                {
                                    articleList.map((v, key) => <li key={v.articleInfo.id}>
                                        <div className="left">
                                            <Lazy className="img" src={v.articleInfo.icon} />
                                        </div>
                                        <div className="right">
                                            <div onClick={() => this.getArticleInfo(v, key)} className="title">
                                                <Tag color={'#00D6AC'}>{v.articleInfo.platform === 1 ? 'Windows' : v.articleInfo.platform === 2 ? 'Mac OS' : 'Android'}</Tag><p>{v.articleInfo.title}</p>
                                            </div>
                                            ...
                                            <div className="bottom">
                                                ...
                                                <span className="mjhu-1"> {v.adminInfo.username} </span>
            ...
            onOk: () =>{
                return new Promise(async (resolve, reject) => {
                    ...
                    if(data.code === 200){
                        ...
                        let newAverageGrade = (Number(allGrade) + Number(newScore)) / (Number(allGradeCount) + 1);
